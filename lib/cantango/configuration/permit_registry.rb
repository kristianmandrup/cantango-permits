module CanTango
  class Configuration
    class PermitRegistry
      def get permit
        raise ArgumentError, "Not an available permit: #{permit}, available: #{CanTango.config.permits.available_types}" if !CanTango.config.permits.available_types.include? permit
